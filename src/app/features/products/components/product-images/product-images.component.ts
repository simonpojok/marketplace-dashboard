...
export class ProductImagesComponent {
    ...
    protected addImage(): void {
        const imageGroup = this.fb.group({
            id: [null],
            image: [''],
            alt_text: [''], // Auto-generated from product name
            is_primary: [false],
    ...
    protected removeImage(index: number): void {
        ...
        // Remove from previews and reindex
    ...
    private processFile(file: File, index: number): void {
        ...
        imageGroup.get('alt_text')?.setValue(file.name.split('.')[0]); // Use filename without extension

        // Set as primary if it's the first image
        if (this.imagesFormArray.length === 1) {
            imageGroup.get('is_primary')?.setValue(true);
        }
    ...
    private updateDisplayOrders(): void {
        for (let i = 0; i < this.imagesFormArray.length; i++) {
            this.imagesFormArray.at(i).get('display_order')?.setValue(i);
        }
    }
